...
qualplots2 <- ggplot(senicdata, aes(x = factor(X8))) + geom_bar(fill = "magenta", width = 0.3) + xlab("US Geographic Region") +
...
quantplots3 <- ggplot(senicquants) + geom_boxplot(aes(x = Obs, y = X3)) + ggtitle("avg. est. prob. of infection, in percent") +
  ylab("Percent") + xlab("No. of observations")
quantplots4 <- ggplot(senicquants) + geom_boxplot(aes(x = Obs, y = X4)) + ggtitle("Routine culturing ratio / 100") +
  ylab("Ratio") + xlab("No. of observations")
quantplots5 <- ggplot(senicquants) + geom_boxplot(aes(x = Obs, y = X5)) + ggtitle("Routine chest X-ray ratio / 100") +
  ylab("Ratio") + xlab("No. of observations")
quantplots6 <- ggplot(senicquants) + geom_boxplot(aes(x = Obs, y = X6)) + ggtitle("avg. no. of beds in hospital") +
  ylab("Beds") + xlab("No. of observations")
quantplots9 <- ggplot(senicquants) + geom_boxplot(aes(x = Obs, y = X9)) + ggtitle("avg. no. of patients in hospital") +
  ylab("Patients") + xlab("No. of observations")
quantplots10 <- ggplot(senicquants) + geom_boxplot(aes(x = Obs, y = X10)) + ggtitle("avg. no. of full-time nurses in hospital") +
  ylab("Nurses") + xlab("No. of observations")
quantplots11 <- ggplot(senicquants) + geom_boxplot(aes(x = Obs, y = X11)) + ggtitle("percentage of facilities in hospital") +
  ylab("Percent") + xlab("No. of observations")
...
#sort(senicdata[,2],index.return = TRUE, decreasing = TRUE)$ix
#High scorers include 47, 53, 104,
